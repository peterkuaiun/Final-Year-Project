...
def Hole_label(img,hole,t = "Hole"):
    ...
    cX = int(M['m10'] / M['m00'])
    cY = int(M['m01'] / M['m00'])
    cv2.putText(img,str(t), (cX,cY),cv2.FONT_HERSHEY_SIMPLEX, 2,(240,240,240),3,cv2.LINE_AA)
    return img
...
#Draw one hole
def Draw_Hole(img,hole,color_1 = (100,205,40),color_2 = (0,0,255),thickness=3,lineType=8,shift=0): 
...
def Draw_AllHoles(img,all_holes,color_1 = (100,205,40),color_2 = (0,0,255),thickness=3,shift=0,label = False):
    for i in range (0,int(len(all_holes))):
        ...
        img = Draw_Hole(img,hole,color_1,color_2,thickness,shift)
        if (label == True):
