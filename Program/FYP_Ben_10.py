"""
Created on Thu Nov  7 14:48:45 2019
...
fname = 'data_4' #The data file name
...
def Hole_label(img,all_holes,t = "Hole"):
    ...
    cX = int(M['m10'] / M['m00'])
    cY = int(M['m01'] / M['m00'])
    cv2.putText(img,str(t), (cX-20,cY+30),cv2.FONT_HERSHEY_SIMPLEX, 3,(200,0,200),3,cv2.LINE_AA)
    return img
...
def Find_holesIP(img,area_min = 0):
#Create the gray Image for Image Processing
    ...
    for i in range(len(holes)):
        area = cv2.contourArea(holes[i])
        if(area >= area_min and area < imgray.size): #The in the range of Area
            all_holes.append(holes[i])
            continue
    return img,all_holes

results_img,all_holes = Find_holesIP(img,area_min)
...
data_info =str("Total Holes:" + str(len(all_holes))+
