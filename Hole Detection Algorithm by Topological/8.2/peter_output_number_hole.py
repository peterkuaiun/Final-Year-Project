...
for index in file_name:
    for fd_name in FD_name_list:
        output_file_name = (r'D:\8.2. Result of Number Of Hole Matching\Sparse\Each FD number of hole\\' + index + '\\')
        folderlist = os.listdir(fd_result_txt + fd_name + '\\' + index + '\\')
        folderlist.sort(key= lambda x:int(x[:-4]))
        for fd_txt in folderlist:
            if not os.path.isdir(output_file_name):
                os.mkdir(output_file_name)
            fd_num_hole = readData(fd_result_txt + fd_name + '\\' + index + '\\' + fd_txt)
